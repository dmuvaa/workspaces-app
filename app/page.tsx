...
const workspaces = [
  ...
  },
  {
    name: "CreativeCorner",
    location: "New York",
    image: "/images/creativecorner.jpg",
    rating: 4.6,
    reviews: 95
  },
  {
    name: "InnovationLab",
    location: "London",
    image: "/images/innovationlab.jpg",
    rating: 4.9,
    reviews: 150
  },
  {
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative h-[600px] flex items-center justify-center text-white">
        ...
        <div className="relative z-10 text-center px-4">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 font-serif">Find Your Perfect Workspace</h1>
          <p className="text-lg md:text-xl mb-8 font-sans">Discover coworking spaces that inspire creativity and productivity</p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-2">
            <Input 
              className="w-full sm:w-64 text-black dark:text-white placeholder-gray-500 dark:placeholder-gray-400" 
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-1/4">
            <Card className="sticky top-4">
              <CardHeader>
                <CardTitle className="text-2xl font-serif">Filter Workspaces</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div>
                    <h3 className="font-semibold mb-3 text-lg font-sans">Amenities</h3>
                    {['WiFi', 'Meeting Rooms', 'Coffee', '24/7 Access'].map((amenity) => (
                      <div key={amenity} className="flex items-center mb-2">
                        <Checkbox id={`amenity-${amenity}`} />
                        <Label htmlFor={`amenity-${amenity}`} className="ml-2 text-sm font-sans">{amenity}</Label>
                      </div>
                    ))}
                  </div>
                  <div>
                    <h3 className="font-semibold mb-3 text-lg font-sans">Office Type</h3>
                    {['Private Office', 'Open Space', 'Meeting Room', 'Virtual Office'].map((type) => (
                      <div key={type} className="flex items-center mb-2">
                        <Checkbox id={`type-${type}`} />
                        <Label htmlFor={`type-${type}`} className="ml-2 text-sm font-sans">{type}</Label>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <section className="w-full lg:w-3/4">
            <h2 className="text-3xl font-bold mb-6 font-serif">Featured Workspaces</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {workspaces.map((workspace) => (
                <Card key={workspace.name} className="overflow-hidden flex flex-col">
                  <div className="relative h-48">
                    <Image src={workspace.image || "/placeholder.svg"} alt={workspace.name} fill style={{objectFit: "cover"}} />
                  </div>
                  <CardContent className="p-4 flex flex-col flex-grow">
                    <h3 className="text-xl font-semibold mb-2 font-serif">{workspace.name}</h3>
                    <p className="text-muted-foreground mb-4 flex items-center text-sm font-sans">
                      <MapPin className="w-4 h-4 mr-1" /> {workspace.location}
                    </p>
                    <div className="flex justify-between items-center mt-auto">
                      <div className="flex items-center">
                        <Star className="w-4 h-4 text-yellow-400 mr-1" />
                        <span className="text-sm font-sans">{workspace.rating.toFixed(1)} ({workspace.reviews} reviews)</span>
                      </div>
                      <Button variant="outline">View Details</Button>
                    </div>
                  </CardContent>
      ...
      <section className="bg-secondary py-16">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-8 text-center font-serif">Popular Locations</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['New York', 'London', 'San Francisco', 'Tokyo', 'Berlin', 'Paris', 'Singapore', 'Sydney'].map((city) => (
              <Link href={`/${city.toLowerCase().replace(' ', '-')}`} key={city} className="text-center">
                <div className="bg-background rounded-lg p-4 hover:shadow-md transition-shadow">
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-12 text-center font-serif">Why Choose Global Workspaces?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              {icon: Coffee, title: "Premium Amenities", description: "Enjoy high-speed WiFi, ergonomic furniture, and unlimited coffee to fuel your productivity."},
              {icon: Users, title: "Vibrant Community", description: "Connect with like-minded professionals and expand your network in our diverse coworking spaces."},
      ...
      <section className="bg-primary text-primary-foreground py-16">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4 font-serif">Ready to find your perfect workspace?</h2>
          <p className="text-xl mb-8 font-sans">Join thousands of satisfied professionals who have found their ideal work environment through Global Workspaces.</p>
          <Button size="lg" variant="secondary">
            Get Started
          </Button>
        </div>
